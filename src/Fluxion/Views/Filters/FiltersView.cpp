...
#include "FiltersView.hpp"
#include "Graphite/Common/UI/ImGuiHelpers.hpp"

#include "Fluxion/Data/Formatters.hpp" // IWYU pragma: keep

DEFINE_LOG_SCOPE(Fluxion::Application::Views::FiltersView);
USE_LOG_SCOPE(Fluxion::Application::Views::FiltersView);
...
std::string_view FiltersView::GetViewName() noexcept
{
    return "FiltersView";
}

std::string_view FiltersView::GetName() const noexcept
{
    return FiltersView::GetViewName();
}
...
void FiltersView::OnAdd()
{
    LOG_SCOPE("::OnAdd()");
}

void FiltersView::OnIterate()
{
    LOG_SCOPE("::OnIterate()");
...
void FiltersView::OnRemove()
{
    LOG_SCOPE("::OnRemove()");
}

inline bool FiltersView::IsActive() const noexcept
{
    return m_application->GetApplicationState().views_active.filters;
}

inline void FiltersView::SetIsActive(bool const open)
{
    m_application->GetApplicationState().views_active.filters = open;
}

inline std::string_view FiltersView::GetDisplayName() const noexcept
{
    return "Filters";
}

void FiltersView::MarkFiltersMetadataDirty()
{
...
void FiltersView::RenderToolbar()
{
    ...
    Graphite::Common::UI::TabItemIconButton(ICON_CI_NEW_FOLDER, "Add Tab", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::AddTab,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
                 .tab_id = std::nullopt, .filter_id = std::nullopt, .condition_id = std::nullopt}});
        MarkFiltersMetadataDirty();
    });

    auto const any_condition_field_none_and_active{[&tabs = app_state.filters.tabs]() {
        for (auto const& tab : tabs.GetFront())
        {
            for (auto const& filter : tab->filters.GetFront())
            {
                for (auto const& condition : filter->conditions.GetFront())
                {
                    if (condition->over_column_display_name == "None" &&
                        (*filter)[Filters::EFilterFlag::IsActive])
                    {
                        return true;
                    }
                }
    ...
    ImGui::BeginDisabled(
        ...
        any_condition_field_none_and_active());
...
void FiltersView::RenderTab(std::shared_ptr<Filters::Tab> tab_ptr)
{
    ...
    Graphite::Common::UI::IconButton(ICON_CI_PLUS, "Add Filter", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::AddFilter,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
    ...
    Graphite::Common::UI::IconButton(ICON_CI_COPY, "Duplicate Tab", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::DuplicateTab,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
                 .tab_id = tab_ptr->id, .filter_id = std::nullopt, .condition_id = std::nullopt}});
    });
    ...
    Graphite::Common::UI::IconButton(ICON_CI_TRASH, "Delete Tab", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::RemoveTab,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
                 .tab_id = tab_ptr->id, .filter_id = std::nullopt, .condition_id = std::nullopt}});
    });
    UIHelpers::Styles::PopRedButton();
...
void FiltersView::RenderFilter(Graphite::Common::Utility::UniqueID const& owning_tab_id, Filters::Filter& filter)
{
    ...
    if (ImGui::BeginDragDropTarget())
    {
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("FILTER_PAYLOAD"))
        {
            ...
            {
                Dispatch(
                    {.type = Actions::FiltersView::EFilterActionType::MoveFilter,
                     .data = Actions::FiltersView::Payloads::MoveFilter{
    ...
    Graphite::Common::UI::IconButton(ICON_CI_COPY, "Duplicate Filter", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::DuplicateFilter,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
    ...
    Graphite::Common::UI::IconButton(ICON_CI_TRASH, "Delete Filter", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::RemoveFilter,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
                 .tab_id = owning_tab_id, .filter_id = filter.id, .condition_id = std::nullopt}});
    });
    UIHelpers::Styles::PopRedButton();
    ...
    Graphite::Common::UI::IconButton(ICON_CI_PLUS, "Add Condition", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::AddCondition,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
                 .tab_id = owning_tab_id, .filter_id = filter.id, .condition_id = std::nullopt}});
    });
...
void FiltersView::RenderCondition(
    ...
    Filters::Condition& condition)
{
    ...
    if (ImGui::BeginDragDropTarget())
    {
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONDITION_PAYLOAD"))
        {
            ...
            {
                Dispatch(
                    {.type = Actions::FiltersView::EFilterActionType::MoveCondition,
                     .data = Actions::FiltersView::Payloads::MoveCondition{
                         .tab_id = owning_tab_id,
    ...
    Graphite::Common::UI::IconButton(ICON_CI_TRASH, "Delete Condition", [&] {
        Dispatch(
            {.type = Actions::FiltersView::EFilterActionType::RemoveCondition,
             .data = Actions::FiltersView::Payloads::FiltersDataModify{
